...
| `s` | Client and/or server | Service code(s) — attribution codes from the payment path; when both declare `s`, core merge concatenates client then server (deduped) so a downstream length cap trims server entries rather than the client's |
...
Throws if `appCode` or any `serviceCodes` entry does not match `^[a-z0-9_]{1,32}$`, or if more than `MAX_SERVICE_CODES` service codes are given.
...
Accepts a single string or an array of strings. Attaches the client's `s` code(s) to every payment payload when registered. When the server declared `builder-code`, the core client merge also preserves the server's `a` code, concatenating any server-declared `s` with the client's (client entries first). Multiple codes are useful for layered clients (e.g. an MCP server acting as middleware) that need to attribute multiple participants. Throws if more than `MAX_SERVICE_CODES` codes are given.

<Note>Facilitators cap the combined `s` to the first **5** entries (`MAX_SERVICE_CODES`) at settlement to bound calldata size; extra entries are silently dropped. `declareBuilderCodeExtension` and `BuilderCodeClientExtension` each reject more than 5 codes at their own layer, but the combined total from both sides can still exceed 5 — keep each side's contribution small.</Note>
